...
namespace LenyaPugachev\PassportEmailLogin;
...
class EmailLoginRequestGrant extends AbstractGrant {
	...
	protected function getUserEntityByRequest( Request $request ) {
		...
		if ( method_exists( $model, 'loginEmail' ) ) {
			$user = ( new $model )->loginEmail( $request );
		} else {
			throw OAuthServerException::serverError( 'Unable to find loginEmail method on user model.' );
		}
